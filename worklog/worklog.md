...
按依赖与收益排。P1~P3 来自 2026-08-29 的使用反馈,优先于原计划剩下的。

**先定三条,免得反复讨论:**

1. **`Profile` 就是「转发配置组」,数据模型已经有了,缺的全在 UI。**
   `tunnels.yaml` 里 profiles 是一等结构,但 `tunnels.all()` 把它拍平成
   `Vec<(profile, forward)>`,`forward_index` 是这张扁平表的下标。于是仪表盘
   连组名都不显示,`a` 一起全起;而 `open_forward_form(blank)` 只把新规则放进
   **当前组**,没有组时硬造一个 `default` —— **不手改 yaml 就永远建不出第二个组**。
   这才是「配置组」真正缺的那块,不是数据结构。

2. **一条规则 = 一个 ssh 进程。不做「一个进程挂多个 `-L`」。**
   ssh 本身支持,但会连带毁掉三样:`ExitOnForwardFailure` 变成一条端口被占就
   整组倒;`Health` 不再与 pid 一一对应,三层灯失去落点;`parse_argv` 返回一个
   三元组要改成 N 个,而按 argv 结构认领正是不能退回模式匹配的那块地基。
   一组 5 个端口开 5 条连接的开销,用 ssh config 的 `ControlMaster` 解 ——
   那是配置问题,不是结构问题。
...
**P2 · 组要能建**(`state.rs` + `mod.rs`)

- `N` 新建组(输入名字)、`R` 重命名当前组。**不加删组键** —— 删掉最后一条规则组
  自然消失(已实现),空组无处存在。
- `c` **克隆选中规则**,bind 端口自动取下一个空闲值。一组多端口时 5 个字段有 4 个
  重复,克隆把「再加一个端口」从填 5 格降到改 1 格 —— 这是「组里涉及多个端口」
  最直接的那把钥匙。
- `n` 新建时若组数 > 1,先选组(现在是静默落进当前组)。

验证:全程不碰 yaml 建出一个 3 端口的组;存盘后 yaml 结构与手写等价。

**P3 · 多选启动**(`state.rs` + `mod.rs` + `ui.rs`)
...
**集合动作只有起和停,没有 toggle。** 勾中两条一起一停时 toggle 该往哪边倒没有答案;
单条 `Enter` 才有资格 toggle。
...
**P4 · 流向图形化** —— 详情面板现在是两行文字(`listen here / exit from kami`)。
...
**P5 · 仪表盘统计** —— 每条:运行时长 + 吞吐速率;汇总:up / stopped / incomplete
...
**P6 · 无主隧道**(2026-08-29 读代码时发现,不在原计划里)

改一条正在跑的规则(bind 6022 → 6024),旧进程还活着、还占着 6022,但 `find()` 按
`(kind, spec, host)` 三项全等匹配,**它直接从面板上消失了** —— 端口被占,却看不到
是谁占的。`scan()` 本来就返回本机全部 ssh 转发进程,只是没匹配上的被丢掉了。
列表底部加一段「无主隧道」:显示 pid 与 spec,可停。这正是本模块的主张 ——
静默失败要看得见。

**P7 · `d` 远端端口发现** —— `ssh <host> ss -tlnH`,降级 `netstat -tln`。
**只监听 loopback 的排前面并高亮**(本机实测 12 条里 10 条如此),因为那些正是只能
靠 `-L` 访问的。与 P2/P3 合起来最划算:发现 kami 上 5 个 loopback 端口 →
`Space` 勾选 → 「存成组」,一次建好。

**P8 · 剩余**

- **`n` 新建 / `c` 克隆 host** —— 三模板(基础 / 经跳板 / 带 IdentityFile);
  克隆追加到文件末尾,不碰现有字节。对 `xx-trade-wsl1..4` 这种只差端口的族群。
- **3b 块内多行编辑** —— `Tab` 切 `tui-textarea`,带 ~80 个 OpenSSH 关键字补全
  与逐键校验。依赖已引入但只用在表单单行字段上。
  **优先级最低** —— 6 字段表单已覆盖 35 个块里的 32 个,这一层是补充。
- **文档** —— `.claude/rules/ssh.md` + CLAUDE.md 模块表 + 本节收尾。
...
风险点:P3 的作用域是隐式模态(靠底栏文案兜)、P5 的 pid 归属、P6 与既有认领逻辑
的边界(无主 = `scan()` 有而 `find()` 无,不能反过来靠模式匹配补)。
